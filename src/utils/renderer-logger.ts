...
class RendererLogger {
  private logToMain(level: 'error' | 'warn' | 'info' | 'debug', message: string, error?: any, extra?: any) {
  ...
  async getRecentLogs(lines = 100): Promise<string> {
    try {
      ...
      this.error('Failed to get recent logs', error as Error);
  ...
  async getLogPath(): Promise<string> {
    try {
      ...
      this.error('Failed to get log path', error as Error);
